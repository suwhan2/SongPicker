...
const StepIndicator = ({ currentStep }:StepIndicatorProps) => {
  const steps = ['기본 정보', '회원 정보', '곡 선택', '가입완료'];
  ...
  return (
    <div className="h-[91px] w-full bg-black border-b border-primary flex items-center justify-center">
      <div className="relative w-full max-w-[600px] pt-6 px-4">
        ...
        {/* 배경 바 */}
        <div className="absolute top-6 left-[12.5%] right-[12.5%] h-[3px] bg-white">
          <div 
        ...
        <ul className="flex justify-between items-start w-full">
          {steps.map((step, index) => (
            <li key={index} className="relative flex flex-col items-center w-[25%]">
              <div 
                className={`w-8 h-8 rounded-full border-2 flex items-center justify-center
                  ${index < currentStep ? 'border-secondary bg-secondary text-white' : 
                    index === currentStep - 1 ? 'border-secondary bg-black' : 'border-white bg-black text-white'}
              ...
              <span 
                className={`text-[10px] px-2 py-1 rounded-full mt-6 whitespace-nowrap
                  ${index < currentStep ? 'bg-secondary text-white' : 
                    index === currentStep - 1 ? 'bg-secondary text-white' : 'bg-white text-gray-400'}
                  transition-all duration-300 ease-in-out`}
              >
                {step}
              </span>
