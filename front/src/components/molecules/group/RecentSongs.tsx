...
import { getSongDetail, SongDetail, registerLike, deleteLike } from '../../../services/songService';
import { getGroupRecentSongs } from '../../../services/historyService';
...
const RecentSongs = ({ teamId, teamName }: RecentSongsProps) => {
  ...
  const totalPages = useMemo(
    () => Math.max(1, Math.ceil(recentSongs.length / itemsPerPage)),
    [recentSongs.length, itemsPerPage]
  );
  ...
  const fetchRecentSongs = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await getGroupRecentSongs(teamId);
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (container && container.dataset.startX) {
        ...
        const maxDelta = window.innerWidth * 0.3; // 최대 이동 거리를 화면 너비의 30%로 제한
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (container && container.dataset.startX) {
        ...
        const threshold = window.innerWidth * 0.2; // 20% 이상 스와이프 시 페이지 전환
    ...
    return Array.from({ length: totalPages }).map((_, pageIndex) => (
      <div key={pageIndex} className="w-full flex-shrink-0">
        <div className="space-y-3  py-3 ">
          {recentSongs.slice(pageIndex * itemsPerPage, (pageIndex + 1) * itemsPerPage).map(song => (
            <MusicItem
              key={song.songId}
  ...
  return (
    <div className="w-full h-full">
      <div className="flex justify-between items-center mb-4">
        <h3 className="w-full text-lg font-semibold">{teamName}그룹의 최근 선곡 리스트</h3>
